...
## 6) public/index.html (ultra‑simple UI)
```html
<!doctype html>
<html lang="en">
<head>
...
<body>
  <header><h1>Empanada Debt 🍽️</h1></header>
  <main>
  ...
  <script>
    ...
    async function loadStats(){
      ...
      $('#stats').innerHTML = `
        <p><strong>I owe:</strong> ${s.iOwe} • <strong>Owe me:</strong> ${s.oweMe} • <strong>Net:</strong> ${s.net}</p>
        <table>
          <thead><tr><th>Friend</th><th>I owe</th><th>Owe me</th><th>Net</th></tr></thead>
          <tbody>${rows || '<tr><td colspan="4">No pending debts 🎉</td></tr>'}</tbody>
        </table>`;
    }
